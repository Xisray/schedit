  ...
  return (
    <div className="flex w-full flex-col-reverse items-center justify-between gap-4 overflow-auto p-1 sm:flex-row sm:gap-8">
      <div className="flex-1 text-sm whitespace-nowrap text-muted-foreground">
        Выбрано{" "}
        {table.getFilteredSelectedRowModel().rows.length.toLocaleString()} из{" "}
        {table.getFilteredRowModel().rows.length.toLocaleString()} строк.
      </div>
      <div className="flex flex-col-reverse items-center gap-4 sm:flex-row sm:gap-6 lg:gap-8">
            ...
            <SelectContent side="top">
              {pageSizeOptions.map((pageSize) => (
                <SelectItem key={pageSize} value={`${pageSize}`}>
                  {pageSize === Infinity ? "Все" : pageSize}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
        ...
        <div className="flex items-center justify-center text-sm font-medium">
          Страница {(table.state.pagination.pageIndex + 1).toLocaleString()} из{" "}
